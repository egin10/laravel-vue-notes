...
export default {
    ...
    methods: {
        async getSubject() {
            let {data} = await axios.get(`/api/subjects/${this.$route.params.slug}`);
            this.form = data.subject;
        },

        async update() {
            this.loading = true;            
            let response = await axios.patch(`/api/subjects/${this.$route.params.slug}/edit`, this.form)
            if(response.status === 200) {
                ...
                this.$router.push({name: 'subjects.table'});
            }
        }
    }
};
